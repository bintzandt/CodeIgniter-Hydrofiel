...
class Home extends _SiteController
{
    ...
    public function check_facebook_posts(){
    	...
    	foreach ($data as $post){
		    ...
		    if (! property_exists($post, 'message')) continue;
    ...
    private function get_image_from_id($id){
	    ...
	    if (sizeof($attachments->data) === 0) return FALSE;
	    if ( ! property_exists($attachments->data[0], 'subattachments')) return FALSE;
	    ...
	    foreach ($images as $image){
		    if ($image->type === 'photo') return array($post_url, $image->media->image->src);
	    }
    }
}