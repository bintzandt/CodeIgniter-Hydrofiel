...
class Upload extends _BeheerController
{
    ...
    public function import_users()
    {
        ...
        if (!$this->upload->do_upload('leden')) {
            ...
            if (($result = $this->profile_model->upload_users($full_path)) > 0) {
                success($result . ' rijen bijgewerkt.');
            } elseif ($result === 0) {
                success('De database was up-to-date.');
            } else {
                error('Er is helaas iets fout gegaan.');
            }
    ...
    public function files()
    {
        ...
        if (($data = $this->upload->do_multi_upload()) && $data !== false) {
            ...
            success('Bestand(en) succesvol geupload!');
        } else {
            if ($data === null) {
                error('Geen bestand(en) geselecteerd');
            } else {
                error($this->upload->display_errors());
            }
        }
    ...
    public function delete($type, $path)
    {
        ...
        if (is_file($file)) {
            unlink($file);
            success('Het bestand is verwijderd!');
        } else {
