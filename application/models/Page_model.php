...
class Page_model extends CI_Model {
	...
	public function view( int $page_id ): PageObject {
		...
		return $query->first_row( 'PageObject' );
	}

	public function get_all() {
		$query = $this->db->get( 'pagina' );

		return $query->custom_result_object( 'PageObject' );
	}
	...
	public function get( $id = null, $array = false ) {
		...
		$query = $this->db->get_where( 'pagina', [ 'id' => $id ] );
		if ( $array ) {
	...
	public function make_room( $submenu, $na ) {
		...
		$this->db->update( 'pagina', [ 'plaats' => $this->get_max_plaats( $to_move ) + 1 ] );
	...
	public function get_id( $submenu, $plaats ) {
		$query = $this->db->get_where( 'pagina', [ 'plaats' => $plaats, 'submenu' => $submenu ] );
		if ( $query->num_rows() > 0 ) {
	...
	public function delete( $page_id ) {
		$this->db->delete( 'pagina', [ 'id' => $page_id ] );
