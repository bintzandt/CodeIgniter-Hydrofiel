...
<?php if( isset( $facebook_posts ) ) {
	foreach( $facebook_posts as $facebook_post ) { ?>
		<div class='container container-item'>
			<div class='col-md-3'>
				<img class="lazy img-responsive no_margin" src="/favicon.ico" data-src="<?= $facebook_post->image ?>"
				     data-srcset="<?= $facebook_post->image ?>" alt="<?= lang( "home_view_on_facebook" ); ?>">
			</div>
			<div class='col-md-9' align='left'>
				<p class="news_content">
					<?= $facebook_post->text ?>
				...
				<p align="center">
					<a href="<?= $facebook_post->url ?>" target="_blank"
					   rel="noopener"><?= lang( "home_view_on_facebook" ); ?></a>
				</p>
			</div>
		</div>
		<hr>
