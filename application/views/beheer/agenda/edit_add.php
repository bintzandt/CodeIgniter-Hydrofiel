...
) : form_open_multipart( '/beheer/agenda/submit', [ "class" => "form-horizontal", "role" => "form" ] ) ?>
<?php if ( $edit_mode ) { ?>
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<input type="text" class="form-control" name="nl_naam" id="nl_naam" placeholder="Naam"
		       value="<?= ( $edit_mode ) ? $event->nl_naam : '' ?>">
	</div>
</div>
<div class="form-group">
	<label for="summernote" class="col-sm-2 control-label">Omschrijving (NL)</label>
	<div class="col-sm-10">
		<textarea class="input-block-level" id="summernote" name="nl_omschrijving"
		          required><?= ( $edit_mode ) ? $event->nl_omschrijving : '' ?></textarea>
	</div>
</div>
<div class="form-group">
	<label class="col-sm-2 control-label" for="en_naam">Naam (EN)</label>
	<div class="col-sm-10">
		<input type="text" class="form-control" name="en_naam" id="en_naam" placeholder="Naam"
		       value="<?= ( $edit_mode ) ? $event->en_naam : '' ?>">
	</div>
</div>
<div class="form-group">
	<label for="engels" class="col-sm-2 control-label">Omschrijving (EN)</label>
	<div class="col-sm-10">
		<textarea class="input-block-level" id="engels" name="en_omschrijving"
		          required><?= ( $edit_mode ) ? $event->en_omschrijving : '' ?></textarea>
	</div>
</div>
<div class="form-group">
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<div class="input-daterange input-group" id="datepicker">
			<input type="text" class="input-sm form-control" name="van" id="van"
			       value="<?= ( $edit_mode ) ? $event->get_formatted_date_string( 'van' ) : '' ?>" />
			<span class="input-group-addon">tot</span>
			<input type="text" class="input-sm form-control" name="tot" id="tot"
			       value="<?= ( $edit_mode ) ? $event->get_formatted_date_string( 'tot' ) : '' ?>" />
		</div>
	</div>
</div>
<div class="form-group">
	...
	<div class="col-sm-10">
		<input type="text" class="form-control" name="link" id="link"
		       placeholder="Link naar het Facebook-evenement of website"
		       value="<?= ( $edit_mode ) ? $event->link : '' ?>">
	</div>
</div>
<div class="form-group">
	<label class="col-sm-2 control-label" for="locatie">Locatie</label>
	<div class="col-sm-10">
		<input type="text" class="form-control" name="locatie" id="locatie" placeholder="Locatie"
		       value="<?= ( $edit_mode ) ? $event->locatie : '' ?>">
	</div>
</div>
<div class="form-group">
	<label for="inschrijven" class="col-sm-2 control-label">Inschrijven mogelijk</label>
	<div class="col-sm-10">
		<label class="radio-inline"><input required type="radio" name="inschrijfsysteem"
		                                   onchange="toggleInschrijf($(this).val())" id="inschrijven"
		                                   value="1" <?= ( $edit_mode && $event->inschrijfsysteem ) ? 'checked' : '' ?>>Ja</label>
		<label class="radio-inline"><input required type="radio" name="inschrijfsysteem"
		                                   onchange="toggleInschrijf($(this).val())"
	...
	<div class="col-sm-10">
		<div class="input-group date">
			<input type="text" class="form-control" name="inschrijfdeadline" id="inschrijf"
			       value="<?= ( $edit_mode && $event->inschrijfsysteem ) ? $event->get_formatted_date_string( 'inschrijfdeadline' ) : '' ?>"><span
	...
	<div class="col-sm-10">
		<div class="input-group date">
			<input type="text" class="form-control" name="afmelddeadline" id="afmeld"
			       value="<?= ( $edit_mode && $event->inschrijfsysteem && $event->afmelddeadline ) ? $event->get_formatted_date_string( 'afmelddeadline' ) : '' ?>"><span
					class="input-group-addon"><i class="glyphicon glyphicon-th"></i></span>
		</div>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<label class="radio-inline"><input required type="radio" name="betalen" id="betalen"
		                                   value="1" <?= ( $edit_mode && $event->betalen ) ? 'checked' : '' ?>>Ja</label>
		<label class="radio-inline"><input required type="radio" name="betalen"
		                                   value="0"
				<?= ( $edit_mode && ! $event->betalen ) ? 'checked' : '' ?>>Nee</label>
	</div>
</div>
<div class="form-group">
	<label for="maximum" class="col-sm-2 control-label">Maximum aantal aanmeldingen</label>
	<div class="col-sm-10">
		<input type="number" value="<?= ( $edit_mode ) ? $event->maximum : 0 ?>" id="maximum" name="maximum"
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<button class="btn btn-primary center-block btn-lg"
		        type="submit"><?= ( $edit_mode ) ? 'Opslaan' : 'Toevoegen' ?></button>
	</div>
</div>
<?= form_close(); ?>
