...
?>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <label class="radio-inline"><input required  <?= ($edit_mode && $page->bereikbaar==='ja')? 'checked': ''?> type="radio" name="bereikbaar" id="bereikbaar" value="ja">Ja</label>
            <label class="radio-inline"><input required  <?= ($edit_mode && $page->bereikbaar==='nee')? 'checked': ''?> type="radio" name="bereikbaar" value="nee">Nee</label>
        </div>
    </div>
    <div class="form-group">
        <label for="zichtbaar" class="col-sm-2 control-label">Zichtbaar in menu</label>
        <div class="col-sm-10">
            <label class="radio-inline"><input required  <?= ($edit_mode && $page->zichtbaar==='ja')? 'checked': ''?> type="radio" name="zichtbaar" id="zichtbaar" value="ja">Ja</label>
            <label class="radio-inline"><input required  <?= ($edit_mode && $page->zichtbaar==='nee')? 'checked': ''?> type="radio" name="zichtbaar" value="nee">Nee</label>
        </div>
    </div>
    <div class="form-group">
        <label for="hoofdmenu" class="col-sm-2 control-label">Hoofdmenu</label>
        <div class="col-sm-10">
            <label class="radio-inline"><input required <?= ($edit_mode && $page->submenu==='A')? 'checked': ''?> type="radio" onchange="update(this.value)" name="hoofdmenu" id="hoofdmenu" value="1">Ja</label>
            <label class="radio-inline"><input required <?= ($edit_mode && $page->submenu!=='A')? 'checked': ''?> type="radio" onchange="update(this.value)" name="hoofdmenu" value="0">Nee</label>
        </div>
    </div>
    <div class="form-group">
        <label for="ingelogd" class="col-sm-2 control-label">Moet ingelogd zijn</label>
        <div class="col-sm-10">
            <label class="radio-inline"><input required <?= ($edit_mode && $page->ingelogd)? 'checked': ''?> type="radio" name="ingelogd" id="ingelogd" value="1">Ja</label>
            <label class="radio-inline"><input required <?= ($edit_mode && !$page->ingelogd)? 'checked': ''?> type="radio" name="ingelogd" value="0">Nee</label>
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <div class="col-sm-10 pull-right">
            <button type="submit" id="submit" class="btn btn-primary center-block btn-lg"><?= ($edit_mode)? 'Opslaan' : 'Toevoegen'?></button>
        </div>
    </div>
<?= form_close(); ?>